...
class Qwen3CoderToolParser(ToolParser):
    ...
    def _get_arguments_config(self, func_name: str, tools: list[dict[str, any]]) -> dict:
        """Extract argument configuration for a function."""
        if tools is None:
            return {}
        for config in tools:
            if "type" not in config or "function" not in config and "name" in config.function:
                continue
            if config["type"] == "function" and config["function"]["name"] == func_name:
                if "parameters" not in config["function"]:
                    return {}
        ...
        if self.in_function:
                ...
                if func_content_end != -1:
                    ...
                    try:
                        parsed_tool = self._parse_xml_function_call(
                            func_content, self.streaming_request.tools if self.streaming_request else None
                        )
                        if parsed_tool:
                    ...
                    if ">" in remaining:
                        ...
                        if param_end_idx != -1:
                            ...
                            converted_value = self._convert_param_value(
                                param_value, self.current_param_name, param_config, self.current_function_name
                            )
            ...
            if self.in_param:
                if self.parameter_end_token in delta_text:
                    ...
                    # Get parameter configuration for type conversion
                    param_config = self._get_arguments_config(
                        self.current_function_name, self.streaming_request.tools if self.streaming_request else None
                    )

                    # Convert the parameter value to the appropriate type
                    converted_value = self._convert_param_value(
                        full_value, self.current_param_name, param_config, self.current_function_name
                    )
